...
import { NewCallsCountProvider } from "./new-calls-count";
...
export default async function AppLayout({ children }: { children: ReactNode }) {
      ...
      <main className="flex min-h-dvh items-center justify-center bg-ground px-6">
        <div className="w-full max-w-[400px] rounded-lg border border-line bg-surface shadow-card p-6">
          ...
          <p className="mt-2 text-[13px] leading-relaxed text-ink-2">
            ...
            <span className="font-medium text-ink">{email}</span>, but this
            account has not been activated for Calman, or has been deactivated.
          </p>
          <p className="mt-2 text-[13px] leading-relaxed text-ink-2">
            A Super Admin or Manager can activate it from Settings → Users.
            Until then there is nothing here for you to see.
          </p>
  ...
  return (
    <UnsavedProvider>
      {/* The pill polls, the rail's badge reads what it found: one number
          between them, since they now blink as one alert (§29.1). */}
      <NewCallsCountProvider>
